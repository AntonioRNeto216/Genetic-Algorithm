...
class DataVisualizer:
    '''
    All logic to visualize data generated by network.

    Attributes:

    All the following attributes are lists to hold values to plot or print
    - ``self._fittests_log``
    ...
    The following attribute holds the path to output of the current execution 
    - ``self._output_path``
    '''
    ...
    def new_generation_data(self, generation_number: int, population: typing.List[Individual]) -> None:
        '''
        Generates all required parameters and saves them in attributes.

        Also, prints the fittest individual of the generation.
    ...
    def generate_files(
        ...
        '''
        Generates all files and shows the graphs.

        Parameters:

        All the following parameters are specified in the GeneticAlgorithm class.
        - ``number_of_generations``
        - ``population_size`` 
        - ``probability_of_crossover`` 
        - ``probability_of_mutation`` 
        - ``tournament_parameter_N``
        - ``tournament_parameter_K``
        '''
